...
class AioVwiki:
  ...
  async def validity_check(
    self, vtuber: str, auto_correct: bool
  ):
    params = 
    x = ""
    if not auto_correct:
  ...
  async def search(self, vtuber: str, limit=10):
    req = await self.session.get(
      f"https://virtualyoutuber.fandom.com/api.php", params={
      "action": "query",
      "srsearch": vtuber,
      "srlimit": limit,
      "list": "search",
  ...
  async def history(self, vtuber: str, auto_correct: bool = True):
    ...
    if hs_tag is not None:
      hs = ""
      next_node = hs_tag.parent.find_next_sibling()
      # r_next_node = hs_tag.parent.find_next_sibling()
